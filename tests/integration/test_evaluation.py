...
def test_admin_evaluate_idea_success(client, db):
    ...
    client.post("/auth/register", json={"email": admin_email, "password": "password"})
    client.post("/auth/register", json={"email": sub_email, "password": "password"})
    ...
    sub_login = client.post("/auth/login", data={"username": sub_email, "password": "password"})
    ...
    admin_login = client.post("/auth/login", data={"username": admin_email, "password": "password"})
    admin_token = admin_login.json()["access_token"]
    
    eval_res = client.patch(
        f"/admin/ideas/{idea_id}/evaluate",
    ...
    view_res = client.get(f"/ideas/{idea_id}", headers={"Authorization": f"Bearer {sub_token}"})
...
def test_submitter_evaluate_idea_forbidden(client):
    ...
    client.post("/auth/register", json={"email": sub_email, "password": "password"})
    login_res = client.post("/auth/login", data={"username": sub_email, "password": "password"})
    token = login_res.json()["access_token"]
    
    # Try to evaluate an idea (doesn't matter which one, endpoint should be blocked by RoleChecker)
    response = client.patch(
        "/admin/ideas/some-id/evaluate",
