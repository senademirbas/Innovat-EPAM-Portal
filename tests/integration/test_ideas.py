...
def test_list_ideas_isolation(client):
    ...
    token_a = client.post("/auth/login", data={"username": "a@example.com", "password": "password"}).json()["access_token"]
    client.post("/ideas", data={"title": "Idea A Title", "description": "Description for Idea A", "category": "AI"}, headers={"Authorization": f"Bearer {token_a}"})
    
    # User B
    client.post("/auth/register", json={"email": "b@example.com", "password": "password"})
    token_b = client.post("/auth/login", data={"username": "b@example.com", "password": "password"}).json()["access_token"]
    client.post("/ideas", data={"title": "Idea B Title", "description": "Description for Idea B", "category": "AI"}, headers={"Authorization": f"Bearer {token_b}"})
    
    # Check User A ideas
    res_a = client.get("/ideas", headers={"Authorization": f"Bearer {token_a}"})
...
def test_view_idea_detail(client):
    ...
    response = client.get(f"/ideas/{idea_id}", headers={"Authorization": f"Bearer {token}"})
